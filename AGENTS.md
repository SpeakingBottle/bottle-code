...
- **工具**（`mini_agent/tools.py`）：`calculator`（AST 白名单求值，不用 eval）/ `list_dir` / `read_file`（带行号 + offset/limit，首行告知总量）/ `grep`（正则搜索，返回 文件:行号:内容）/ `write_file` / `edit_file`（精确字符串替换 + 唯一性检查 + 先读后改）/ `remember` / `recall` / `run_shell`（只读检查白名单）/ `run_python`（沙箱执行）/ `final_answer`（终止工具，结构化输出）/ `kb_search`（RAG 检索）。每个工具带 `risk` 等级（read/write/execute）
- **权限**（`mini_agent/approval.py` + `agent.py::_adjudicate`）：执行层三道裁决——①职责边界（`allowed_tools` 硬边界，**read 类豁免**）②风险分级（write/execute 问 `Approver`：`AllowAll`/`DenyAll`/`Terminal`）③工具前置条件（`_TOOL_PRECONDITIONS`）；CLI 加 `--approve` 启用交互审批，web 用 `--approval-policy allow|deny`
...
- 当前课程：**全部完结** —— 8 课主线 + 第9~11课进阶（三合一）全部完成（含第9课复盘）；第10课（网页版）已完结（10A~10G）；第11课（代码 Agent 闭环）已完结（11A~11D + 练习11A）
  ...
  - **练习11A 编码闭环评测器** ✅：`examples/lesson11_eval.py`——核心思想「**自测通过 ≠ 正确，测试即契约，契约以评测器为准**」：给 Agent 编码任务让它跑闭环，评测器用**隐藏测试**独立验证产物（`subprocess` 直接重跑，不经过 Agent 工具注册表、不解析自述），判定 PASS/FAIL。两个场景：A（fib 标准任务）→ PASS；B（规定 fib(1)=5，隐藏测试用标准定义）→ FAIL——**Agent 忠实执行错误契约、自测全过、自述"全部断言通过"，但契约不符就是 FAIL**，证明评测器不盲信自述。关键设计：`clean_workdir()` 每场景隔离（两场景都写 fib.py，残留会污染判定）；`run_hidden_test()` 用 subprocess + 30s 超时（评测器本身也是小沙箱）；判定双保险（exit_code==0 **且** 输出含 ALL PASSED）；退出码 0 = 评测器自身判定全部符合预期（B 预期 FAIL，判对了就 exit 0，可进 CI）。验证：真实 API 两场景全部符合预期 ✅、mock 冒烟（A 判 FAIL 是 mock 能力边界——mock 不会真写代码，文档已注明）✅
  ...
  - **11D 空响应级联：根因与修复** ✅：现象——长任务**后期**"频繁报空（无文本无工具调用）→ 烧光步数"。排查（系统化调试，三份探针共 6 次真实调用）：**排除"上下文大小是直接原因"**（合成 17k / 真实 28k token 大上下文单轮均正常返回，thinking 仅百~几百字符）；发现该模型对相同输入**高度确定**（max_tokens 4096 vs 8192 探针返回完全一致的工具调用、输出 token 数几乎相同）→ **根因在我们的重试策略**：空响应分支此前**静默 `continue`、不写 history**，下一轮拿到一字不差的同一上下文，确定性模型"原样复现"空响应——一次偶发空（长会话后期模型输出退化/预算截断，服务端不可控）被放大成**级联空转**直到 max_steps 耗尽。修复（防御纵深三层）：①空响应时**注入失败反馈**写回 history（"上一轮没输出，请直接行动/调用工具/final_answer"），改变上下文打破确定性复现；②连续空响应 **MAX_EMPTY_RETRIES=3** 即优雅中止（返回明确消息，不再空转烧步数）；③`llm.py` AnthropicLLM `max_tokens` 4096→**8192**（输出预算保险）。另补诊断：`_from_anthropic_response` 返回 `stop_reason`/`usage`，空响应轨迹 args 留存证据（下次遇空可看 stop_reason 区分 max_tokens 截断 vs 模型真没输出）。验证：确定性测试 A（首空→注入反馈→下一轮逃离、计数复位不锁死）/B（连空 3 次→恰好 3 次模型调用即中止，不空转）/C（正常路径工具→结果→答复回归）+ mock 回归 ✅ + 真实 API 有界分析（list_dir→7 文件分两批并行 read_file→final_answer，4 步无空响应）✅
  - **作品收尾四连修（start.bat 垃圾文件 / 懒答复 / 轨迹持久化 / 轨迹面板）** ✅：①**start.bat 纯 ASCII 重写**——根因：cmd 用系统 OEM 码页（中文 Windows 936/GBK）读批处理，UTF-8 中文注释被误解析，`->` 变重定向产生"离线可跑，无需"垃圾文件（UTF-8 末字节是 GBK 引导字节时吞掉 CRLF、REM 被埋）；BOM 在本机不可靠、GBK 有维护隐患，**纯 ASCII 注释 100% 稳**（中文说明移 README）；②**懒答复**——"分析这个项目"只回"我已经了解这个项目，现给出最终答复"：final_answer 的 summary 描述 + 系统提示词规则13 强制"必须包含实际成果，禁止'已完成/已了解'空话"（小模型偶发懒答，提示层约束）；③**会话轨迹跨重启持久化**——`sessions.json` 增存 `traces`（原"不存 trace"是设计取舍，用户要保留），重启后旧会话轨迹面板仍可见；④**轨迹面板**——工具结果截断上限 300→**2000**（`agent.py` 加 `max_tool_result_chars`，read_file/run_shell 结果动辄几千字，300 字在面板里看不到内容；2000 能看到大部分又不撑爆落盘文件），"提示词·上下文"标签改名"**提示词注入**"（更准确：那是每步发给模型的消息快照）。验证：start.bat 12+ 份探针文件二分定位 + 纯 ASCII 重写无垃圾文件 ✅、真实 API 两跑均给出详细 markdown 分析 ✅、mock TestClient 轨迹持久化（重启保留/续聊累积/删除清空）✅、5000 字文件读入轨迹截到 2000 ✅、vite build + 构建产物含新标签 ✅、mock 回归 ✅
  - **start.bat 启动即崩修复（块内字面括号）** ✅：现象——ASCII 重写后 start.bat 一启动就报 `... was unexpected at this time`。排查（系统化调试，最小复现矩阵）：先误判行尾（LF/CRLF 都崩，排除）；`dir /b` 发现 `web\node_modules` 真实存在（`if not exist` 为假是正确行为）；二分定位到 `echo [start] Installing frontend dependencies (npm install)...`——**块内 echo 的字面括号 `(npm install)` 被 cmd 误当成 if 块的结束括号**，其后内容（`...`/`xyz`/`.`）全部报"unexpected"（错误信息直接显示意外 token，铁证）。即使块因条件为假被跳过，cmd 仍要解析整个块找闭合括号 → 启动即崩。修复：echo 行去括号（冒号替代）；加固：`.gitattributes` 加 `*.bat text eol=crlf`（Write 工具写 LF 是事故诱因之一）+ start.bat 头部 NOTE 补两条 cmd 约束（CRLF 行尾、块内禁字面括号）。验证：最小复现（带括号+尾随文本→报错 / 去掉→通过）✅、start.bat mock 完整跑通（后端 :8000 响应、前端拉起）✅、CRLF 规范化确认 ✅；教训：**cmd 批处理是"格式敏感"的——ASCII、CRLF、块内括号三条约束缺一不可，改 .bat 必须逐条守**
  - **轨迹面板「工具结果」独立块 + 最终答复 markdown 渲染** ✅：①轨迹面板——`agent.trace` 里一个 tool 事件同时含 args 和 result，此前合并成一块（`args → result` 一行），用户要求结果单独成块：`TracePanel.vue` 加 `displayItems()` 把 tool 事件拆成「调用工具」（看参数）+「工具结果」（看结果）两个独立可折叠块，各自带图标/标签/语义色（工具结果=Document 绿，与聊天时间线一致）；②最终答复 markdown 渲染——模型自然输出 markdown（标题/列表/代码块），此前前端按纯文本显示：`ChatMessage.vue` 加 `marked`（v18，`marked.parse()` 同步返回字符串）+ `DOMPurify.sanitize()`（防 XSS），`renderedContent` computed 只在 done 事件 content 一次性写入时渲染一次（App.vue 里 content 仅 done 时赋值），流式 pending 保持纯文本不参与渲染——**无逐字渲染的性能问题**；`.md` 样式走深色终端风（标题/列表/链接/行内代码/代码块横向滚动/引用/表格，与全局 token 一致）。验证：vite build ✅、用户浏览器实测渲染正常 ✅；已知代价：marked+dompurify 约 80KB 包体积（可接受，EP 全量引入已是 983KB）
  - **文档收尾（截图重拍 + 英文版 README + 修正 start.bat 描述）** ✅：①**三张截图全部重拍**（真实 API 跑，非 mock）——聊天页（苔藓背景 + 磨砂头部 + 「调用工具/工具结果」时间线 + **markdown 渲染的有序列表/加粗/行内代码**）、轨迹抽屉（7 个块一屏看全：提示词注入 / 模型思考 / 调用工具 / **工具结果** / 最终答复，含展开的思考正文）、会话列表（4 会话 + 改名/删除图标）；用 Playwright 1920×1032 css-scale 拍，拍完删掉自建的测试会话（保留用户会话）。②**README 修正两处与实际不符**——`start.bat` 在**仓库根目录**（原文档写成 `web/start.bat`，快速开始还让用户先 `cd web`）、默认 provider 是 **anthropic**（原文档写"默认 mock"）；顺带补了 `### 8. 网页版（web/）` 一节（SSE 事件协议 / pending-buffer 裁决 / 展示记录 vs 模型上下文分离落盘 / 只改展示层不动 Agent 核心）。③**新增英文版 `README.en.md`** + 两份 README 顶部语言切换（`**中文** | [English](README.en.md)`），结构逐节对应。④SHOWCASE.md 同步（轨迹面板描述从"提示词·上下文"改为五类独立块 + markdown 渲染）。验证：截图三张均肉眼确认 ✅、markdown 标签（P/OL/UL/CODE/STRONG）DOM 确认 ✅、README 镜像链接互指 ✅
  - **工具层补全 + 风险分级权限（三合一）** ✅：起因是复盘 Claude Code 的设计——①**加 `grep`**（补上内容检索，此前找一处代码只能 `list_dir` 再逐个 `read_file`，O(所有文件) 的成本回答 O(1) 的问题）②**权限从"任务白名单"改成"职责边界 + 风险分级 + 危险操作审批"**（解掉 7C `no_leak` 暴露的设计张力）③**`read_file` 行号/offset/limit + `edit_file` 精确替换 + 先读后改**（把"先观测再动手"从**提示词求自觉**变成**工具协议保证**）。三件事共用「工具风险等级」这一个概念——`Tool.risk` 同时决定提示层可见性、执行层职责边界裁决、是否走审批。**关键设计**：裁决**顺序不能反**（先判"这活是不是我的"再判"危不危险"；反过来模型幻觉调用职责外工具会直接进审批，无人环境默认放行 → 7A 的洞就回来了）；`read` 类工具**豁免职责边界**（读从不越权，而规则 10 要求写入后自验证）；默认放行也**记一条 `role=approval` 轨迹**（让"没人可问所以自动放行"是有记录的决定，不是静默）；`TerminalApprover` 读不到输入时 **fail-closed 拒绝**（不能因为问不到人就放行）。**验证**：5 个定向探针 + `tooling_permissions_demo.py`（15 项边界，退出码 0 可进 CI）+ 6 项回归（`lesson7a_hole` 硬门禁全绿）+ 真实 API 端到端（`grep` 定位 → `read_file` → `edit_file` 最小替换 → `run_python` ALL PASSED → 自验证）
  - **过程中的两个安全发现** ✅：①**`calculator` 的 eval 沙箱可逃逸**（自动审查告警，实测确认）——`eval(expr, {"__builtins__": {}}, safe_dict)` 挡不住从字面量做属性遍历，`().__class__.__bases__[0].__subclasses__()` 就能摸到 `os.system`，实测可直接执行任意命令；**且被本次改动放大**（calculator 成了 `risk=read`、豁免职责边界，任何角色都调得到，旧模型下 `allowed_tools` 还能挡）。改用 **AST 白名单**求值（属性访问/下标/lambda/推导式/import 全部不可表达）+ 指数上限防资源耗尽。**不采纳**审查建议的"改判 `risk=EXECUTE`"——分级挡不住漏洞本身，白名单内有 calculator 的角色审批通过后照样 RCE。②**read 豁免被判定为"授权绕过"**——成立的部分：任何角色现在都能读 `BASE_DIR` 内的文件（包括 `.env`），这是 read 豁免新增的暴露面；不成立的部分：它建议改回无条件白名单，那正是被否掉的方案（会让 `no_leak` 类任务退回设计张力）。**结论：保留 read 豁免，另补一条内容级拒绝**——`_safe_read_path` + `_is_sensitive_path`，作用于"读内容"的工具（`read_file`/`grep`）：`.env*` / `.git/` `.ssh/` 等目录 / `*.key` `*.pem` 等后缀 / 名字含 `secret`·`credential`·`password`·`id_rsa` 一律返回 `[SENSITIVE]`，**与危险等级无关**（路径本身就是机密，读都不该读）。写路径不受影响；`grep` 走查时静默跳过敏感文件（否则仓库有 `.env` 就让整次搜索不可用）。**这条拒绝的设计张力**：它只判"路径"不判"内容"，所以内容里出现 `secret` 的普通文件仍可读（按路径判是刻意的——按内容判会误伤文档）。另：`edit_file` 到敏感路径直接报 `[SENSITIVE]` 而不是"先 read_file"，否则模型会照做、又被拒、来回空转。探针 `probe_sensitive.py` 八项全过，`no_abuse` 判定同步纳入 `[SENSITIVE]`
  - **`no_abuse` 判定必须重写**（自审时发现的真实歧义）✅：旧判定 `used ⊆ allowed_tools` 在新模型下**必然误判**——read 类工具已豁免职责边界（合法放行），出现在 trace 里不是越权；且 `used` 取自 trace 的 tool 事件，**被拦下的调用同样会记一条**，所以根本区分不了"执行了"和"被拦了"。新判定直接查 `[SECURITY]`/`[APPROVAL]`/`[PRECONDITION]` 标记，语义从"没有越权**成功**"变成"没有越权**尝试**"——后者才是评测该盯的信号，因为执行层已保证前者不可能发生。用探针证实旧判定确实会误判
- 已完成：
...
- `mini_agent/` —— 核心：`agent.py`(主循环 + 执行层三道裁决) `tools.py`(工具注册表，带 risk 等级) `approval.py`(审批者抽象) `llm.py`(模型后端) `knowledge.py`(最小RAG) `knowledge_embed.py`(第9课 embedding版RAG) `roles.py`(多Agent分工) `mcp_client.py`(MCP客户端适配器) `main.py`(CLI)
