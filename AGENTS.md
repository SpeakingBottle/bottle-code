...
- 当前课程：**第10课（CodeOps 网页版）**（进行中）—— 8 课主线已完成，进入第9~11课进阶阶段
  ...
  - **10D 修补（低透明度 + 502 提示 + 轨迹排版）** ✅：①背景遮罩再调低（rgba .55→.68，苔藓更明显）+ 给消息文本加暗色 `text-shadow`（照片上垫字——亮度升了、文字依然可读）；②**502 根因**=后端没在 :8000 跑、Vite 代理转发失败——前端 `send()` 对 502/网络错（`Failed to fetch`）给可行动提示（"后端未启动…请运行 start.bat"），运行 `start.bat` 或手动起 `web/server.py` 即解决；③轨迹抽屉排版——每步一张浅色卡片（左边条=类型色），head=【图标·标签·元信息靠右 nowrap】，body 用细分割线隔开，顶部加「本会话 N 个步骤」统计，`meta()` 耗时格式化到 .2f。验收：后端 mock 起、前端提问不再 502 ✅、截图（亮苔藓+文字可读 / 轨迹卡片排版）✅、vite build ✅
- 已完成：
