...
> 证据：`codeops_demo` 一次运行落盘 **42 条审计事件**；网页轨迹抽屉按会话隔离展示"提示词·上下文/调用工具/答复"。
...
### 8. 网页版（第10课）
SSE 流式（后端把 `run(stream=True)` 事件转成 `data:` 推送）+ Vue 3 前端；会话持久化到磁盘，刷新续聊。
> 证据：浏览器全链路验证（输入→tool 事件→result→delta→done）；会话列表/轨迹抽屉/改名/删除均可用；`mock` 离线即玩。
